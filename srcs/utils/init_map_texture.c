...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/04/19 12:15:16 by mubulbul          #+#    #+#             */
/*   Updated: 2026/04/19 12:15:17 by mubulbul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_map_texture(t_all *all)
{
	...
	while (i < 6 && map[i])
	{
		if (ft_strncmp(map[i], "NO", 2) == 0 && !all->texture->no)
			all->texture->no = ft_strdup(map[i] + 2);
		else if (ft_strncmp(map[i], "SO", 2) == 0 && !all->texture->so)
			all->texture->so = ft_strdup(map[i] + 2);
		else if (ft_strncmp(map[i], "WE", 2) == 0 && !all->texture->we)
			all->texture->we = ft_strdup(map[i] + 2);
		else if (ft_strncmp(map[i], "EA", 2) == 0 && !all->texture->ea)
			all->texture->ea = ft_strdup(map[i] + 2);
		else if (ft_strncmp(map[i], "F", 1) == 0 && !all->texture->f)
			all->texture->f = ft_strdup(map[i] + 1);
		else if (ft_strncmp(map[i], "C", 1) == 0 && !all->texture->c)
